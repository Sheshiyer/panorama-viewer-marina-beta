...
export function FloorSidebar({ activeFloor, setActiveFloor }: Props) {
  ...
  return (
    <aside
      className={cn(
        "fixed right-6 top-32 z-40 flex flex-col gap-3",
        collapsed ? "w-14" : "w-48"
      )}
    >
      <button
        ...
        {floors.map((f) => {
          ...
          return (
            <button
              ...
              className={cn(
                pressable,
                "relative w-full flex flex-col items-center justify-center px-3 py-2 rounded-xl text-slate-200",
                active
                  ? "bg-gradient-to-r from-amber-400 to-orange-500 text-slate-950 shadow-lg"
                  : "bg-white/0 hover:bg-white/5"
              )}
            >
              <span className="text-lg font-bold">{f.floorNumber}F</span>
              {!collapsed && (
